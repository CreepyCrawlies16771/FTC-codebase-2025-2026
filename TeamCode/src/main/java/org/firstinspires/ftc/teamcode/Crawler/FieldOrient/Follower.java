...
public abstract class Follower extends LinearOpMode {
    ...
    protected double currentLookahead = RobotConfig.FieldOriented.defaultLookAheadDistance;
     ...
     */
    public void runPath(List<Pose2d> path, List<PathMarker> markers) throws InterruptedException {
    ...
    public void runPath(List<Pose2d> path) throws InterruptedException {
        runPath(path, null);
    }
    ...
    private void driveToPoint(Pose2d currentPose, Pose2d targetPoint) {
        ...
        double turnPower = angleError * RobotConfig.FieldOriented.STEER_P;

        // Use the distance to the target to calculate speed
        double distance = Math.hypot(deltaX, deltaY);
        double driveSpeed = Math.max(RobotConfig.FieldOriented.MIN_POWER, Math.min(0.7, distance * RobotConfig.RobotOriented.Kp));
    ...
    private boolean isPathFinished(List<Pose2d> path) {
        ...
        return Math.hypot(finalPoint.getX() - currentPose.getX(),
                finalPoint.getY() - currentPose.getY()) < RobotConfig.FieldOriented.FINISH_THRESHOLD_CM;
    }
